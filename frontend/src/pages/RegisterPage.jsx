...
export default function RegisterPage() {
  ...
  const termoSentinelRef               = useRef(null);

  useEffect(() => {
    api.get("/departments").then(({ data }) => setDepartments(data)).catch(() => {});
  }, []);

  // Desbloqueia as checkboxes quando o sentinel abaixo do PDF fica visível (usuário chegou ao fim)
  useEffect(() => {
    if (termoLido || !termoSentinelRef.current) return;
    const obs = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) setTermoLido(true); },
      { threshold: 0.5 }
    );
    obs.observe(termoSentinelRef.current);
        ...
        <form onSubmit={handleSubmit} className="card p-6 space-y-5">
          ...
          <div className="border-t border-gray-100 dark:border-gray-800 pt-4 space-y-3">
            ...
            <div>
              ...
              {/* iframe com o PDF oficial — IntersectionObserver no sentinel abaixo detecta quando o usuário chegou ao fim */}
              <iframe
                src="/termo-responsabilidade.pdf#toolbar=0&navpanes=0"
                title="Termo de Responsabilidade SEJUSC"
                className="w-full rounded-xl border border-gray-200 dark:border-gray-700"
                style={{ height: "480px" }}
              />
              {/* Sentinel: fica visível quando o usuário rola a página até abaixo do PDF */}
              <div ref={termoSentinelRef} className="h-1" aria-hidden="true" />
            </div>
